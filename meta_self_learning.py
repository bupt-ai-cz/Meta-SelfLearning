...
from dataset import hierarchical_dataset, AlignCollate, Batch_Balanced_Dataset, Batch_Balanced_Dataset0, SelfTrainingDataset, self_training_collate
...
def train(opt):
    ...
    valid_dataset, valid_dataset_log = hierarchical_dataset(root=opt.valid_data, opt=opt, pseudo=True)
    ...
    self_training_dataset = SelfTrainingDataset([], [])
    self_training_loader = None
    ...
    if 'CTC' in opt.Prediction:
        ...
        criterion = torch.nn.CrossEntropyLoss(ignore_index=0).to(device)  # ignore [GO] token = ignore index 0 忽略0类别，所有长度不足25的标签均用0进行填充
    # loss averager
    loss_avg = Averager()
    ...
    if opt.saved_model != '':
        try:
            ...
            if opt.continue_epoch != -1: start_iter=opt.continue_epoch
            pass
    ...
    while(True):
        if train_dataset.has_pseudo_label_dataset(): meta_target_index = random.randint(0, opt.source_num)
        else: meta_target_index = random.randint(0, opt.source_num - 1)
        
        meta_target_index = random.randint(0, opt.source_num)
        if train_dataset.has_pseudo_label_dataset(): meta_target_index = opt.source_num


        old_state_dict = deepcopy(model.state_dict())
        weight_buffer, buffer_buffer = [], []
        first_order_weight_buffer, first_order_buffer_buffer = [], []
        ...
        for mix_iter in range(opt.inner_loop_iter + 1):
            ...
            if mix_iter == opt.inner_loop_iter - 1:
                first_order_weight_buffer.extend([deepcopy(p.grad) for p in model.parameters()])
                first_order_buffer_buffer.extend([deepcopy(p) for p in model.buffers()])  
，
            if mix_iter == opt.inner_loop_iter: 
                weight_buffer.extend([deepcopy(p.grad)for p in model.parameters()])
                buffer_buffer.extend([deepcopy(p) for p in model.buffers()])
                break
        model.load_state_dict(old_state_dict)

        old_param = list(map(lambda p: p, model.parameters()))
        for old, new, first_order in zip(old_param, weight_buffer, first_order_weight_buffer): 
            old.grad = new
        ...
        if train_dataset.has_pseudo_label_dataset():
            image_tensors, labels = train_dataset.get_meta_test_batch(opt.source_num)
        else:
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            with open(f'./saved_models/{opt.exp_name}/log_train.txt', 'a') as log:
                model.eval()
                with torch.no_grad():
                    valid_loss, current_accuracy, current_norm_ED, preds, confidence_score, labels, infer_time, length_of_data, all_imgs, all_labels, all_confidences, all_gts = validation(
                        model, criterion, valid_loader, converter, opt, self_training=True)
                    test_loss, test_current_accuracy, test_current_norm_ED, test_preds, test_confidence_score, test_labels, test_infer_time, test_length_of_data = validation(
                        model, criterion, test_loader, converter, opt)
                model.train()
                print(len(all_imgs), len(all_labels), len(all_confidences))
                print(all_imgs[0].shape)
                # training loss and validation loss
                loss_log = f'[{iteration+1}/{opt.num_iter}] Train loss: {loss_avg.val():0.5f}, Valid loss: {valid_loss:0.5f}, Elapsed_time: {elapsed_time:0.5f}'
                current_test_log = f'{"Test_Current_accuracy":17s}: {test_current_accuracy:0.3f}, {"Test_Current_norm_ED":17s}: {test_current_norm_ED:0.2f}'
                loss_avg.reset()
                ...
                if test_current_accuracy > test_best_accuracy:
                    test_best_accuracy = test_current_accuracy
                    torch.save(model.state_dict(), f'./saved_models/{opt.exp_name}/test_best_accuracy.pth')
                if test_current_norm_ED > test_best_norm_ED:
                    test_best_norm_ED = test_current_norm_ED
                    torch.save(model.state_dict(), f'./saved_models/{opt.exp_name}/test_best_norm_ED.pth')
                ...
                right_pseudo_label_counts = 0

                for conf, img, label, gt in zip(all_confidences, all_imgs, all_labels, all_gts): 
                    if conf < opt.pseudo_threshold: continue
                    label = label[: label.find('[s]')]
                    self_training_imgs.append(img)
                    self_training_labels.append(label)
                    if gt == label: right_pseudo_label_counts += 1
                
                self_training_dataset_length = len(self_training_imgs)
                if self_training_dataset_length > 0 and current_accuracy >= opt.warmup_threshold:
                    pseudo_log = 'Generating Self-Training Dataset, contains {} imgs, {} of them have right labels'.format(self_training_dataset_length, right_pseudo_label_counts)
                    print(pseudo_log)
                    log.write(pseudo_log + '\n')
                    self_training_dataset = SelfTrainingDataset(self_training_imgs, self_training_labels)
                    train_dataset.add_pseudo_label_dataset(self_training_dataset, opt)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', type=float, default=1, help='beta to weigh the second order derivative loss')
    ...
    parser.add_argument('--pseudo_threshold', type=float, default=0.8, help='The threshold of using pseudo labels')
    parser.add_argument('--warmup_threshold', type=float, default=27.5, help='The threshold of using pseudo labels')
    parser.add_argument('--source_num', type=int, default=3, help='the number of source domain dataset')
    ...
    parser.add_argument('--continue_epoch', type=int, default=-1)
